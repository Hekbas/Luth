...
namespace Luth
{
    ...
    void InspectorPanel::DrawEntityComponents(Entity m_SelectedEntity)
    {
        ...
        // Temporary shim: route PointLight through the registry. Other components
        // still use the legacy DrawComponent<T> switch; fully flipped in sub-task D.
        for (const auto& desc : ComponentDrawerRegistry::GetDrawers()) {
            if (desc.Type == typeid(PointLight))
                desc.Draw(m_SelectedEntity);
        }
