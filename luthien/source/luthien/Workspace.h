...
namespace Luth
{
    ...
    // Workspace = ImGui dock layout (.ini) + sidecar JSON holding the per-panel visibility set.
    // Pair format keeps the .ini ImGui-native; sidecar evolves freely. Active name lives in
    // EditorSettings::activeLayout.
    namespace Workspace
    {
        // Sidecar JSON shape: { "panel_open": { "<window-id>": <bool>, ... } }.
        // LoadJson leaves outPanelOpen untouched when the file is missing; callers treat that as
        // "no sidecar; keep existing in-memory state" so legacy .ini-only workspaces don't clobber
