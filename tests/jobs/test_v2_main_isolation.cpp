...
//
// Invariant: the main thread (worker index 0) busy-spins on counters but never
// enters the worker steal loop. If it did, glfwPollEvents() would block whenever
// the scheduler picked up a heavy job. This test dispatches N jobs and records
// which worker index ran each one; the main thread's index (0) must not appear.
...
TEST_CASE_FIXTURE(LuthTests::JobSystemFixture,
                  "V2 main thread never executes worker jobs [smoke]")
{
    ...
    Luth::JobSystem::Dispatch(N, 1, RecordTid, &data, &c, "RecordTID",
