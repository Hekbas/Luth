...
namespace Luth
{
                               ...
                               DirectionalLightShadowParams& outShadow) const
    {
        ...
        for (auto [entity, wt, dl] : dirView.each())
        {
            ...
            outLights.dirLight.direction = glm::normalize(-glm::vec3(wt.Matrix[2]));
            outLights.dirLight.color     = dl.Color;
            outLights.dirLight.intensity = dl.Intensity;

            outShadow.castShadows           = dl.CastShadows;
            outShadow.shadowBias            = glm::vec4(dl.ShadowBias[0], dl.ShadowBias[1], dl.ShadowBias[2], dl.ShadowBias[3]);
            outShadow.shadowNormalBias      = glm::vec4(dl.ShadowNormalBias[0], dl.ShadowNormalBias[1], dl.ShadowNormalBias[2], dl.ShadowNormalBias[3]);
            outShadow.splitLambda           = glm::clamp(dl.SplitLambda, 0.0f, 1.0f);
            outShadow.shadowDistance        = dl.ShadowDistance;
            outShadow.stabilizeCascades     = dl.StabilizeCascades;
            outShadow.cascadeBlendWidth     = glm::clamp(dl.CascadeBlendWidth, 0.0f, 1.0f);
        ...
        if (!foundDir)
        {
            outLights.dirLight.direction = glm::normalize(glm::vec3(1.0f, 1.0f, 0.5f));
            outLights.dirLight.color     = glm::vec3(1.0f);
        ...
        {
            if (count >= 64) break;
            outLights.pointLights[count].position  = glm::vec3(wt.Matrix[3]);
