...
namespace Luth
{
    // VK_NV_device_diagnostic_checkpoints marker registry. The driver treats the marker as opaque;
    // a human-readable name must be recoverable when dumping checkpoints after a TDR. Names are interned
    // once for the process lifetime, leaked by design: this is a diagnostic tool, not a steady-state primitive.
