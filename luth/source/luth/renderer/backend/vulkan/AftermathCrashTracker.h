...
// NVIDIA Nsight Aftermath GPU crash-dump tracker. On a device-lost (TDR) it writes a
// .nv-gpudmp (+ .nvdbg shader info) capturing the faulting shader, the page-fault address, and —
// with VK_NV_device_diagnostics_config resource tracking — the offending resource. This is the only
