...
namespace Luth
{
    ...
    // Owns Set 0 (Global UBO + IBL samplers + GTAO final + GTAO UBO) layout.
    // Per-frame UBO is rebound to a fresh GPUTaggedPageAllocator region in
    // UpdateUBO; the Grid set's binding 0 shares the same region (atomic
    // 2-set write — see arch/rendering-pipeline.md). The IBL textures and
    // GTAO sampler/final-view are owned by other subsystems and passed in
    // via GlobalViewWriteContext at AllocateViewResources time.
    ...
    public:
        ...
        // Caches cascades + shadow params; computes viewProjection; rebinds
        // Set 0 binding 0 + Grid set binding 0 to a fresh per-frame UBO region.
