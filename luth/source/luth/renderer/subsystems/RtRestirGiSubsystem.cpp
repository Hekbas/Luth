...
namespace Luth
{
    namespace {
        ...
        // Spatial-pass push constants (shared pcRange). neighbours+radius bit-packed; geomTableBDA
        // feeds the final-visibility trace's alpha test.
        struct GiSpatialPC {
            Mat4 invViewProj;
        ...
        // Fixed push-constant range shared by the four GI pipelines; 128 B (Vulkan min) leaves headroom
        // for the largest struct (spatial 104 B) plus later growth, mirroring the DI k_RestirPCSize.
        constexpr u32 k_RestirGiPCSize = 128;
    }
    ...
    void RtRestirGiSubsystem::Init(RenderPipeline& pipeline)
    {
        ...
        // slimNormal sampler, b2 reservoir SCRATCH (initial -> temporal in-place, r/w SSBO), b3 GI
        // storage image, b4 reservoir HISTORY = the spatial buffer (read SSBO), b5 motion sampler,
        // b6 spatial-output reservoir (same buffer as b4). All stable per-view.
        ...
        // Sets: 0 = global (UBO b0 + TLAS b6), 1 = light SSBO, 2 = pass-local (temporal/shade layout).
        const std::vector<VkDescriptorSetLayout> layouts = {
            m_Pipeline->GetGlobal().GetSetLayout(),
            m_Pipeline->GetLighting().GetSetLayout(),
            m_SetLayout,
        };
        // Initial + spatial add set 3 = Material SSBO + set 4 = bindless textures (secondary-hit
        // material fetch; spatial's final-visibility alpha test). Temporal/shade trace no rays and
        // keep the 3-set layout.
        ...
        VkPushConstantRange pcRange{ VK_SHADER_STAGE_COMPUTE_BIT, 0, k_RestirGiPCSize };
        ...
        m_SpatialPipeline = std::make_unique<VKComputePipeline>(
            m_SpatialSpv, initialLayouts, std::vector<VkPushConstantRange>{ pcRange });
    ...
    bool RtRestirGiSubsystem::OnShaderReloaded(const std::string& name, const std::vector<u32>& spv)
    {
        ...
        // Initial + spatial keep their 5-set layout (Material + bindless), mirroring Init.
        std::vector<VkDescriptorSetLayout> initialLayouts = layouts;
        initialLayouts.push_back(MaterialSystem::GetDescriptorSetLayout());
        initialLayouts.push_back(VulkanContext::Get().GetBindlessSet().GetLayout());
        VkPushConstantRange pcRange{ VK_SHADER_STAGE_COMPUTE_BIT, 0, k_RestirGiPCSize };
                                                      ...
                                                      RG::ResourceHandle slimMotion)
    {
        ...
        // (read+write) and spatial (read) so the RG chains the barriers. Undefined import is correct:
        // fully overwritten, no cross-frame read.
        ...
        rg.AddComputePass<GiInitialData>(
            ...
            [&, this](GiInitialData& data, RG::RenderPassBuilder& builder) {
                ...
                RG::BufferDesc bd{ "RestirGiReservoirScratch", scratchRes.size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT };
        ...
        // the scratch candidate in-place, reweighted by the reconnection Jacobian. The history is the
        // per-view spatial buffer, imported ONCE here in the state it was LEFT in last frame
        // (StorageBufferWrite, NOT Undefined: the RG only makes last frame's write available if the
        // read barrier's src carries a real access mask; Undefined -> srcAccessMask=0 -> no
        // availability -> stale read, temporal never accumulates. see arch/rendering-pipeline.md).
        // Its handle threads into spatial's WriteBuffer so the RG emits the temporal-read ->
        // spatial-write WAR barrier on the same node (re-importing would alias distinct nodes).
        // SCRATCH threads through reservoirHandle (read+write): initial->temporal RAW barrier.
        // No AS barrier; temporal traces no rays.
        ...
        rg.AddComputePass<GiTemporalData>(
            ...
            [&, this](GiTemporalData& data, RG::RenderPassBuilder& builder) {
                ...
                RG::BufferDesc histBd{ "RestirGiReservoirSpatial", spatialRes.size, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT };
        ...
        // disk neighbours into the spatial/history buffer (b6), each reused neighbour reweighted by the
        // reconnection Jacobian + RTXDI BASIC bias correction, then traces one final-visibility ray on
        // the selected sample (5-set layout: Material + Bindless for the alpha test; the initial pass's
        // AS-build -> COMPUTE barrier covers this same-queue trace). Reads b2 read-only (neighbour reads
        // must see un-modified values, never in-place). The scratch handle ends here:
        // ReadBuffer(reservoirHandle) is its last consumer (temporal->spatial RAW barrier). WriteBuffer
        // on the SAME node temporal read (spatialHandle) yields the WAR barrier; the result persists
