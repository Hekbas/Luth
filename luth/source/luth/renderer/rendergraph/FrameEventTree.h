...
namespace Luth::RG
{
    ...
    struct EventNode
    {
        ...
        // Last leaf draw index reachable in this subtree (Unity-style scrub semantics: clicking a
        // Group/Pass/Cascade snaps the draw slider to this value). UINT32_MAX = no Draw descendants.
        // For Draw nodes this equals drawIndex.
        u32  lastDrawIndex      = UINT32_MAX;

        std::vector<EventNode> children;
    };

    // Build the hierarchical tree from a captured frame's flat metadata. Pure function; does not touch GPU
    // state. Safe to call from FinalizeCapture after all CapturedPass / CapturedDrawCall records are populated.
