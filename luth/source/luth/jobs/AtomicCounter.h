...
namespace Luth::JobSystem
{
    ...
    struct AtomicCounter
    {
        ...
        // Public counter primitives. Execute/Dispatch manage the counter
        // internally; these are for external schedulers (e.g. the Jolt
        // JobSystem adapter) that need to manipulate it directly while
        // still cooperating with WaitForCounter's V5 fiber-yield path.
        //   Increment: plain shifted add (n counts).
        //   Decrement: routes through the same wake-waiters path as job
        //              completion, including the busy-bit serialization
