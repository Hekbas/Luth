...
namespace Luth::Physics
{
    ...
    void LuthJobSystemForJolt::LuthBarrier::AddJob(const JobHandle& handle)
    {
        ...
        // SetBarrier returns false if the job has already completed
        // (mBarrier was atomically set to cBarrierDoneState). In that case
        // OnJobFinished will never fire for this barrier, so we must not
    ...
    LuthJobSystemForJolt::LuthJobSystemForJolt(JPH::uint maxJobs, JPH::uint maxBarriers)
    {
        ...
        // Cap our advertised concurrency below Luth's worker count so that
        // non-physics game-stage jobs always have free workers to run on.
        // GetStats() requires Luth::JobSystem::Init() to have been called;
    ...
    {
        // Allocate from the free list. ConstructObject returns an index or
        // cInvalidObjectIndex when full. We yield and retry under pressure
        // (matches JobSystemThreadPool's behavior; if hit often, raise maxJobs).
        JPH::uint32 index;
        for (;;)
        {
        ...
        // JobHandle's ctor AddRef's the job. Construct it before queuing in
        // case the job runs and completes synchronously on QueueJob.
    ...
    {
        // Per Jolt's contract, the barrier is empty (all jobs finished) by
        // the time the user calls DestroyBarrier — they must have waited.
    ...
    {
        // The whole point of this adapter: route Jolt's wait through Luth's
        // V5 inline-execution + fiber-yield path. The calling fiber will steal
        // queued Jolt jobs from its local deque (up to depth 4) and then yield,
        // freeing the OS thread for other ready fibers.
    ...
    {
        // We're storing the job for asynchronous execution — take a ref so
        // it survives until the trampoline runs. The trampoline Releases
        // after Execute. (See Jolt's JobSystem.h doc: "If you store the job
        // in your own data structure you need to call AddRef().")
        job->AddRef();
    ...
    void LuthJobSystemForJolt::TrampolineFn(Luth::JobSystem::JobArgs args)
    {
        ...
        // Job::Execute() runs the job function and, at the end, atomically
        // swaps mBarrier to cBarrierDoneState and calls Barrier::OnJobFinished
        // (which our LuthBarrier uses to decrement its counter and wake any
        // fibers waiting in WaitForJobs → WaitForCounter).
        job->Execute();
        // Drop the ref we took in QueueJob. When the last ref drops (which
        // happens here if the user already let their JobHandle expire),
        // FreeJob is called via Job::Release().
        job->Release();
    }
}
