...
namespace Luth
{
    ...
    // ═══════════════════════════════════════════════════════════════
    //  Serialization Helpers (extracted from SceneSerializer patterns)
    // ═══════════════════════════════════════════════════════════════
    ...
    namespace CommandUtil
    {
        ...
        void DeserializeEntitySubtree(Scene& scene, const json& snapshot)
        {
            ...
            for (auto& [child, parentUUIDStr] : parentLinks) {
                ...
                if (it != uuidToEntity.end() && it->second.IsValid()) {
                    // Remove from roots first (CreateEntity added it there)
                    scene.RemoveFromRoots(child);
                    child.SetParent(it->second);
                } else {
                    // Parent is outside the subtree — find in scene by UUID
        ...
        {
            // Entity was just deserialized and added to roots by CreateEntity.
            // Set parent first (which also handles root list removal).
            if (parent.IsValid()) {
                entity.SetParent(parent);

                // Now reorder within parent's children to the desired index
                ...
                // Should already be in roots (from CreateEntity).
                // Reorder within root list.
                entity.RemoveParent(); // Ensure it's a root
