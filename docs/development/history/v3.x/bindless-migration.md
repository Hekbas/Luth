...
**Series:** `rt-renderer`, first effort. Mode A series-coalesced — `Version.h` MAJOR bump to `v3.0.0` (series start; raised minimum HW requirement to RT-capable GPU and the broader raster → RT-first architecture flip justify MAJOR per Godot 3→4 precedent), tag-only, no Release.
...
| F | **Wrap-up + `Version.h` bump.** This history file. `arch/rendering-pipeline.md` Set 1 row updated + BDA pattern subsection added. `rt-renderer.md` Progress Tracker A.1 row → done. **`Version.h` bumped 2.12.0 → 3.0.0** (series-start MAJOR — fundamental rendering architecture change + raised minimum HW requirement; Godot 3→4 precedent for rendering-rewrite MAJOR bumps; bump landed on a separate `chore: bump Version.h to v3.0.0` commit immediately before the merge). `--no-ff` merge into `main` + `v3.0.0` tag. | this commit |
