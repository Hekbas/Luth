...
| v3.7.5 | `async-blas-build` | Two main-thread load freezes removed: ray-tracing BLAS builds deferred onto the async-compute TLAS pass (non-blocking upload poll, ready-generation folds late builds in, per-mesh raster/skinning gating); asset imports made fire-and-forget with a per-UUID in-flight guard + atomic artifact writes, so cold-cache first import no longer blocks. Closes #171 | 2026-07-10 |
