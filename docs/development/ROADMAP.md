...
| M3 composition | `triplanar` ✅ / `detail-maps` ✅ / `parallax-occlusion` ✅ / `decals` | M–L | Triplanar + DetailNormal shipped with `effect-layer` (v3.7.3); POM shipped on `feat/surface-detail` (#175: TangentViewDir + SampleLod seam, repurposed-specular height slot, GraphParallax raster==RT); UV-space decals next in the sub-arc. |
